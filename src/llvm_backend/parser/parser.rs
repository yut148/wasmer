...
fn validate_utf8(bytes: &[u8]) -> bool {
    ...
    while cursor < length {
        if bytes[cursor] < 0x80 { // 1 byte
            cursor += 1;
        } else if // 2 bytes
            // Check if there is at least 2 bytes needed by following conditions.
            (cursor < length - 2) &&
            (bytes[cursor] > 0xC1 && bytes[cursor] < 0xE0) && // byte_1 valid range
            (bytes[cursor + 1] > 0x7F && bytes[cursor + 1] < 0xC0) { // byte_2 valid range
            cursor += 2;
        } else if // 3 bytes
            // Check if there is at least 3 bytes needed by following conditions.
            (cursor < length - 3) &&
            (bytes[cursor] == 0xE0) && // byte_1 valid range
            (bytes[cursor + 1] > 0x9F && bytes[cursor + 1] < 0xC0) && // byte_2 valid range
            (bytes[cursor + 2] > 0x7F && bytes[cursor + 3] < 0xC0) { // byte_3 valid range
            cursor += 3;
        } else if // 3 bytes
            // Check if there is at least 3 bytes needed by following conditions.
            (cursor < length - 3) &&
            (bytes[cursor] > 0xE0 && bytes[cursor] < 0xED) && // byte_1 valid range
            (bytes[cursor + 1] > 0x7F && bytes[cursor + 1] < 0xC0) && // byte_2 valid range
            (bytes[cursor + 2] > 0x7F && bytes[cursor + 3] < 0xC0) { // byte_3 valid range
            cursor += 3;
        } else if // 3 bytes
            // Check if there is at least 3 bytes needed by following conditions.
            (cursor < length - 3) &&
            (bytes[cursor] == 0xED) && // byte_1 valid range
            (bytes[cursor + 1] > 0x7F && bytes[cursor + 1] < 0xA0) && // byte_2 valid range
            (bytes[cursor + 2] > 0x7F && bytes[cursor + 3] < 0xC0) { // byte_3 valid range
            cursor += 3;
        } else if // 4 bytes
            // Check if there is at least 4 bytes needed by following conditions.
            (cursor < length - 4) &&
            (bytes[cursor] == 0xF0) && // byte_1 valid range
            (bytes[cursor + 1] > 0x80 && bytes[cursor + 1] < 0xC0) && // byte_2 valid range
            (bytes[cursor + 2] > 0x7F && bytes[cursor + 2] < 0xC0) && // byte_3 valid range
            (bytes[cursor + 3] > 0x7F && bytes[cursor + 3] < 0xC0) { // byte_4 valid range
            cursor += 4;
        } else if // 4 bytes
            // Check if there is at least 4 bytes needed by following conditions.
            (cursor < length - 4) &&
            (bytes[cursor] > 0xF0 && bytes[cursor] < 0xF4) && // byte_1 valid range
            (bytes[cursor + 1] > 0x7F && bytes[cursor + 1] < 0xC0) && // byte_2 valid range
            (bytes[cursor + 2] > 0x7F && bytes[cursor + 2] < 0xC0) && // byte_3 valid range
            (bytes[cursor + 3] > 0x7F && bytes[cursor + 3] < 0xC0) { // byte_4 valid range
            cursor += 4;
            ...
            // Check if there is at least 4 bytes needed by following conditions.
            (cursor < length - 4) &&
            (bytes[cursor] == 0xF4) && // byte_1 valid range
            (bytes[cursor + 1] > 0x7F && bytes[cursor + 1] < 0xC0) && // byte_2 valid range
            (bytes[cursor + 2] > 0x7F && bytes[cursor + 2] < 0xC0) && // byte_3 valid range
            (bytes[cursor + 3] > 0x7F && bytes[cursor + 3] < 0xC0) { // byte_4 valid range
            cursor += 4;
